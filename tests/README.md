...
Create tests in the `tests` directory, in files named `test_*.py`, with test
functions named with a `test_` prefix. Copy the closest existing file for the shape
of a test: `test_solve_api.py` for an end-to-end solve, `test_field_api.py` for a
field builder, `test_derivatives.py` for an adjoint check.

The verification benchmarks in `examples/verification/` are also run as tests, via
`tests/verification/`. Add a new benchmark there rather than here when it has a
published or closed-form reference value; see `examples/verification/README.md`.
